...
## 2026-08-22 — FE-8: 背景の色味を作り直す（デュオトーン + 中央通路）

- **やったこと**（[#35](https://github.com/TakuyaMizukami7/Hackathon/issues/35)）:
  FE-7 の背景が「色が UI と被って見にくい / キャッチーさが足りない」だったので作り直した。
  触ったのは `public/lens-bg.svg` と `bias-filter.css` の背景ブロックだけ
  - 支配色を cyan から **マゼンタ ←→ シアンのデュオトーン**に振った
  - **周辺減光をやめて「中央通路」に置き換えた**（後述）
  - 絵の見せ場（レンズ 2 つ・グリッド）を左右の余白へ寄せた
  - グリッドをマゼンタにして**地平線を白熱**させた。ここが一番キャッチー
  - 粒子ノイズを **0.4 → 0.16** に、走査線を 0.16 → 0.11 に落とした
  - 38 秒かけてごくゆっくり漂わせる（`transform` のみ・reduce で停止）
- **失敗から学んだこと（ここが本題）**:
  - **FE-7 は構図が逆だった。** 周辺減光を掛けて中央を明るく残す＝
    「一番明るい場所に本文が乗る」構図になっていた。しかも色が UI と同じ cyan だったので、
    タイトルの光彩も ▽ ボタンも背景に溶けた。
    → **明るいのは左右の余白、暗いのは本文が乗る中央**。周辺減光を「中央通路」に置き換えた
  - **背景の主役色は UI のアクセント色と別にする。** UI が cyan なら背景はマゼンタ。
    同じ色にすると、どれだけ減光しても手前が沈む
  - **灰色に濁る原因は粒子ノイズだった。** 全面に 0.4 のグレーノイズを乗せていたので、
    彩度を上げる前にまずこれを落とす必要があった
- **決めたこと**:
  - 中央通路の濃さは CSS 変数 `--bf-corridor` 1 つで持つ。
    通路の停止点 3 か所が同時に動くので、読みにくいときはここだけ触ればよい
  - 通路の幅は `calc(50% ± 372px)` の px 指定。狭い画面では計算結果が負になって
    全幅に広がる（本文も全幅になるので構図としては正しい）。
    ただし真っ黒になるので `max-width: 860px` で濃さを **0.91 → 0.72** に下げる。
    ちょうどグリッドの消失点が本文の裏に来るので、薄くしても画になる
  - 漂いは `transform` だけにした。コンポジタで完結するので再描画が起きない
- **検証済み（ヘッドレス Chrome + CDP、`?mock=local`）**:
  1440×900 と 390×844 の実ビューポートで初期表示 / 4 件展開 / 最下部スクロールを確認。
  タイトルと ▽ が背景から明確に浮くこと、4 ペルソナの本文（特に羊皮紙）が読めること、
  390px で真っ黒にならないことを目視確認。
  `prefers-reduced-motion` は computed style で **`animation-name: none` かつ
  1.2 秒後も `transform` が変化しない**ことを確認（通常時は変化する）。
  `npm run build` も通り、`dist/lens-bg.svg`（8047B）が出ることを確認。`npm run check` 通過
- **未検証**: Railway 本番URLでの見え方
- **次やること**: 本番デプロイ後にデモ機で確認
- **相方への申し送り**:
  - `public/lens-bg.svg` を**中身ごと差し替えた**（パスと参照方法は FE-7 のまま）
  - `src/shared/types.ts`・サーバー側・`src/index.css` はいずれも**変更なし**
  - 背景が明るすぎ／暗すぎたら、`.bf::before` の **`--bf-corridor`** だけ触ればよい
  - **PR #33（最新ニュース取得ボタン）の上に乗せている。** `.bf__chip--fetch` とは競合なし

## 2026-08-22 — FE-7: 背景に「レンズ／別次元」のアートワークを敷く

- **やったこと**（[#31](https://github.com/TakuyaMizukami7/Hackathon/issues/31)）:
  ベタ塗りだった背景に、画面いっぱいのアートワークを入れた
  - **`public/lens-bg.svg` を新規作成**（5.8KB、自前の SVG）。レンズの虹彩（同心円 8 本）、
    絞り羽根（六角形 2 枚）、屈折した光の筋、消失点つきのグリッド、ブランドの ▽ の透かし。
    ペルソナ 4 色（cyan / violet / amber / green）は「光の玉」としてだけ置いた
  - `.bf::before` を `position: fixed; inset: 0; z-index: -1` で viewport に敷き、
    上に **粒子 → 走査線 → 周辺減光 → 減光幕** を CSS で重ねた
  - `.bf-item` / `.bf__input` を少し透過 + `backdrop-filter: blur()` にして、
    背景が透ける「レンズごしに見ている」状態にした
- **決めたこと**:
  - **`src/index.css`（グローバル）には足さない。** `body` に背景を付けるのが素直だが、
    あそこは 2 人が触る共有ファイルなので `.bf` の疑似要素で viewport を覆う形にした
  - **粒子（グレイン）は 160px の小さいタイルの繰り返し。** 画面サイズの `feTurbulence` は
    高 DPI 機だとラスタライズが重くなる。絵の SVG 側にはノイズを入れていない
  - **絵の明るさは SVG 側で持たず、CSS の減光幕で調整する。** 絵と減光を分けておくと、
    読みにくいときに CSS の 1 行だけで直せる
  - **チップには `backdrop-filter` を掛けない。** 枚数があるわりに効果が小さい
  - **未入力時のボタンを `opacity` で薄めるのをやめた。** 背景が入ったことで
    ▽ の形ごと絵に溶けて消えてしまったので、暗い側に振った塗りを残す方式に変えた
- **検証済み（ヘッドレス Chrome + CDP、`?mock=local`）**:
  1440×900 と 390×844 の実ビューポートで、初期表示 / 4 件展開 / 最下部までスクロールを確認。
  羊皮紙・真っ黒・ダークグレーの本文がいずれも背景の上で読める。横スクロールなし。
  **`npm run build` → `dist/` を本番と同じ Hono サーバーで配信し、
  `GET /lens-bg.svg` が 200 / `image/svg+xml` / 5855B で返ることを確認**。`npm run check` 通過
- **未検証**: Railway 本番URLでの見え方。実機（デモ機）での `backdrop-filter` の描画負荷。
  重いようなら `.bf-item` の `backdrop-filter` を外すのが一番効く（透過だけは残る）
- **次やること**: 本番デプロイ後にデモ機で表示確認。カクつくようなら上記の blur を外す
- **相方への申し送り**:
  - **`public/lens-bg.svg` を新規追加した。** CSS からは `url('/lens-bg.svg')` で参照している
  - `src/shared/types.ts` とサーバー側は**変更なし**。`src/index.css` も**変更なし**
  - `.bf::before` が `z-index: -1` の固定レイヤーを 1 枚張っている。
    `.bf` の中で `z-index` を使うときは 0 以上にすること

## 2026-08-22 — FE-6: アプリ名「オルタナ▽レンズ」への UI 改修

- **やったこと**（[#29](https://github.com/TakuyaMizukami7/Hackathon/issues/29)）:
  アプリ名の正式決定に合わせて、**CSS の装飾とテキストだけ**で世界観を作り直した。
  HTML の構造も JS のロジックも変えていない（唯一の例外は後述の `data-persona` 1 属性）
  - **テキスト**: タイトル `Bias Filter ▽` → `オルタナ▽レンズ`（`index.html` の `<title>` も）、
    主役ボタン `▽ 展開する` → `▽ オルタナ・レンズを通す`
  - **主役ボタン**: `clip-path: polygon(0 0, 100% 0, 50% 100%)` で**巨大な逆三角形**に切り抜き、
    透過背景 + `backdrop-filter: blur(7px)` で**すりガラス**、ホバーで cyan の **pulse**。
    `.bf__actions` を縦積みにして**中央**に置いた（文字数カウントは右端に逃がした）
  - **ペルソナごとの「別次元」**: `.bf-item__body-inner` だけを 4 通りに差し替えた。
    楽観=丸ゴシック+オレンジのグラデーション / 陰謀=monospace+黒+スキャンライン+ネオングリーン /
    歴史家=明朝+羊皮紙+セピア / 投資家=シャープな sans-serif + ダークグレー単色
- **決めたこと**:
  - **光彩は `box-shadow` ではなく `filter: drop-shadow()`。** `clip-path` は box-shadow ごと
    切り落とすので、三角形の輪郭に沿う drop-shadow でないと光らない
  - **`--accent` をこの機能の中だけ cyan に再定義した**（`.bf` に 1 行）。
    グローバルの `src/index.css` は触っていない。チップのホバー・不定形バー・「もう一度」が
    まとめてレンズの色になる
  - **見出し行（`.bf-item__head`）は 4 件とも同じ見た目のまま**にした。
    開いた瞬間の落差を見せ場にしたいので、豹変させるのは本文だけ
  - 羊皮紙のパネルだけ背景が明るくなるので、`.bf-item__meta` / `.bf-item__tag` /
    未点灯のバイアスメーターもセピアに上書きした（グレーのままだと読めない・点灯して見える）
- **検証済み（ヘッドレス Chrome + CDP、`?mock=local`）**:
  900px と 390px の両方で 4 件展開まで確認。**横スクロールなし**（390px で scrollWidth=390）、
  ボタンは 390px でも 1 行（高さ 100px）。ホバーで `filter` が実際に脈打つこと、
  `prefers-reduced-motion: reduce` では**脈打たず光彩が静止する**ことを computed style で確認。
  `npm run check` 通過
- **未検証**: 本番URLでの見え方（ローカルの vite dev でしか見ていない）。
  **丸ゴシックは Windows に無い**ので、楽観主義者だけ環境によっては標準 sans-serif に落ちる
  （字間を広げて柔らかさを補ってはいる）。当日のデモ機で一度見ておくこと
- **次やること**: 本番デプロイ後、デモ機のブラウザで 4 パネルのフォントを目視確認する
- **相方への申し送り**:
  - `src/shared/types.ts` とサーバー側は**変更なし**
  - **`PerspectiveItem.tsx` の `<li>` に `data-persona={id}` を 1 属性だけ足した。**
    ペルソナ別の CSS を当てるためのフックで、分岐やスタイルの実体は全部
    `bias-filter.css` 側にある。TSX にロジックは増えていない
  - `.bf` の中でだけ `--accent` を `#22d3ee` に上書きしている。
    `.bf` 配下で `var(--accent)` を使うと青ではなく cyan になる
